...
mw = 'm'            ## 'm' = men, 'w' = women
yrstart = 2000      ## first season to calculate totals
yrend = 2023        ## last season to calculate totals
match_min = 0      ## minimum number of matches (with matchstats)
                    ## a player must have to be included for a given year
input_path = 'data/'  ## path to the single-season results csv files
...
for yr in range(yrstart, yrend + 1):
    ...
    for pl in qualifs:
        ...
        ace_rate = aces / float(svpt) if svpt != 0 else 0
        df_rate = dfs / float(svpt) if svpt != 0 else 0
        firstin_rate = firstin / float(svpt) if svpt != 0 else 0
        first_win = firstwon / float(firstin) if firstin != 0 else 0
        second_win = secondwon / (svpt - float(firstin)) if (firstin != 0 & svpt != 0) else 0
        spw = firstwon + secondwon
        spw_rate = spw / float(svpt) if svpt != 0 else 0

        ## raw return stats
        vaces, vdfs, retpt, vfirstin, vfirstwon, vsecondwon = sum_row[10:16]
        ## calculate more aggregates
        rpw = retpt - vfirstwon - vsecondwon
        rpw_rate = rpw / float(retpt) if retpt != 0 else 0

        tpw_rate = (spw + rpw) / (float(svpt) + retpt) if (firstin != 0 & svpt != 0) else 0
        dom_ratio = rpw_rate / (1 - spw_rate) if spw_rate != 0 else 0
